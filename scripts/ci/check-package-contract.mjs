...
for (const packageDirectory of packageDirectories) {
  ...
  if (!['zero-dependency', 'bare-react-native', 'native-contained'].includes(manifest.ruban?.runtimePolicy)) {
  ...
  if (manifest.ruban?.runtimePolicy === 'bare-react-native' && Object.keys(manifest.dependencies ?? {}).length > 0) {
    fail(`${label}: bare-react-native packages cannot declare runtime dependencies`);
  }
