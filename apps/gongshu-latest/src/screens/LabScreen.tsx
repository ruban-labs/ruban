...
import {
  ...
  RubanScreen,
  primitiveStyles,
} from '../components/RubanPrimitives';
...
type Props = NativeStackScreenProps<RootStackParamList, 'DeveloperLab'>;
...
function StepButton({
  ...
  return (
    ...
    >
      <Text style={[styles.stepButtonText, { color: colors.ink }]}>
        {label}
      </Text>
    </TouchableOpacity>
  );
    ...
    <RubanScreen testID="screen-lab">
      ...
      >
        <View style={styles.labTitleRow}>
          <Text style={[primitiveStyles.cardTitle, { color: colors.ink }]}>
            Bar
          </Text>
          <Text
            testID="lab-bar-value"
        ...
        <View style={styles.controls}>
          <StepButton
            label="− 10"
            onPress={() => adjust(setBarProgress, -0.1)}
          />
          <StepButton
            label="+ 10"
            onPress={() => adjust(setBarProgress, 0.1)}
      ...
      >
        ...
        <View style={styles.controls}>
          <StepButton
            label="− 10"
            onPress={() => adjust(setCircleProgress, -0.1)}
          />
          <StepButton
            label="+ 10"
            onPress={() => adjust(setCircleProgress, 0.1)}
      ...
      >
        ...
        <View style={styles.visualRow}>
          ...
          />
          <CircleSnail
            animating={indeterminateAnimating}
            size={82}
            thickness={7}
            color={[colors.accent, colors.ink, colors.success]}
          />
        </View>
        <View style={styles.controls}>
          <StepButton
            label="− 10"
            onPress={() => adjust(setPieProgress, -0.1)}
          />
          <StepButton
            label="+ 10"
            onPress={() => adjust(setPieProgress, 0.1)}
      ...
      <View style={[styles.linkNote, { backgroundColor: colors.accentSoft }]}>
        <Text style={[styles.linkNoteLabel, { color: colors.accent }]}>
          REPRODUCE THIS STATE
        </Text>
        <Text selectable style={[styles.linkText, { color: colors.ink }]}>
          ruban://dev/lab/progress?bar={barProgress.toFixed(2)}&amp;circle=
  ...
  if (props.route.params?.tool !== 'progress') {
    ...
    return (
      <DesignPlaygroundScreen
        ...
        onOpenProgress={() =>
          props.navigation.navigate('DeveloperLab', {
            tool: 'progress',
            bar: '0.64',
            circle: '0.42',
            pie: '0.76',
