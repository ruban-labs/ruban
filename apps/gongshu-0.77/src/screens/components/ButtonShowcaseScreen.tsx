...
const buttonStates = ['default', 'pressed', 'loading', 'disabled', 'full'] as const;
...
export default function ButtonShowcaseScreen({route, navigation, onBack}: Props): React.ReactElement {
  const theme = parseOption<RubanThemeMode>(route.params.theme, ['light', 'dark'], 'light');
  const variant = parseOption<ButtonVariant>(route.params.variant, buttonVariants, 'primary');
  ...
  const deepLink = `ruban-rn077://components/button?theme=${theme}&variant=${variant}&size=${size}&state=${state}`;
