...
export class OverlayCoordinator<Value> {
  ...
  present(entry: OverlayEntry<Value>): void {
    const activeIndex = this.active.findIndex(candidate => candidate.id === entry.id);
    if (activeIndex >= 0) {
    ...
    const queuedIndex = this.queued.findIndex(candidate => candidate.id === entry.id);
    if (queuedIndex >= 0) {
  ...
  dismiss(id: string): void {
    ...
    if (queuedLength === this.queued.length && activeLength === this.active.length) {
      return;
    }
  ...
  private shouldQueue(entry: OverlayEntry<Value>): boolean {
    if (this.phase === 'dismissing' || Object.keys(this.blockerCounts).length > 0) {
      return true;
    }
