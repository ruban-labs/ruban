...
官网与 App 共用视觉体系，不各自重新设计。`design/theme-colors.json` 是浅色／深色
基础色和语义色的共同源，通过生成器产出 RN 主题包和官网 CSS。修改源文件后再生成，
禁止手改生成结果；使用 `pnpm design:theme:check` 检测漂移。

两端沿用相同的字体层级原则、克制的圆角、硬朗边缘、语义图标和简洁文案。
新增界面使用语义色，不另造页面专属色值。组件以 App Playground 为参照，品牌使用
已批准的 SVG 母版。系统字体与响应式布局允许有意差异：官网主视觉不是 App 导航标题。

实现分别使用 Astro/CSS 和 React Native，不为共用而强行统一组件代码。
验收时对照两端的浅色／深色、窄屏／宽屏代表截图，检查层级、密度、对齐、对比度、
焦点／触摸区域与溢出。视觉验收由人判断，不变成脆弱的像素级 CI 门禁。
被接受的设计决定先记录在本文，再同步应用到两端。
